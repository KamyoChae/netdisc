...
namespace Project
{
    public class JavaScriptHelper
    {
        ...
        //成功提示
        public static void Success(System.Web.UI.Page page, string mess)
        {
            string jscode = "setTimeout(function(){layer.msg('" + mess + "', { icon: 1, time: 1500, maxWidth: 200, maxHeight: 120 });},300);";


            page.ClientScript.RegisterStartupScript(page.GetType(), "message", jscode, true);
        }
        //成功提示后转向
        public static void Success(System.Web.UI.Page page, string mess,string redirectUrl)
        {
            string url = !string.IsNullOrEmpty(redirectUrl) ? ("location.href = '" + redirectUrl + "';") : "";
            string jscode = "setTimeout(function(){layer.msg('" + mess + "', { icon: 1, time: 1500, maxWidth: 200, maxHeight: 120 }, function () { " + url + " });},300);";
            

            page.ClientScript.RegisterStartupScript(page.GetType(), "message", jscode, true);
        }
        //异常提示
        public static void Error(System.Web.UI.Page page, string mess)
        {
            string jscode = "setTimeout(function(){layer.msg('" + mess + "', { icon: 2 });},300);";
            

            page.ClientScript.RegisterStartupScript(page.GetType(), "message", jscode, true);
        }
        //异常提示后转向
        public static void Error(System.Web.UI.Page page, string mess, string redirectUrl)
        {
            string url = !string.IsNullOrEmpty(redirectUrl) ? ("location.href = '" + redirectUrl + "';") : "";
            string jscode = "setTimeout(function(){layer.msg('" + mess + "', { icon: 2, time: 1500 }, function () { " + url + " });},100);";
